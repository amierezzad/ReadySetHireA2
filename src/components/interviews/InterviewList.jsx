...
const InterviewList = () => {
	...
	const fetchApplicantCounts = async (interviewId) => {
		...
		return {
			...
			notStarted: applicants.filter(
				(applicant) => applicant.interview_status === "Not Started"
			).length,
			total: applicants.length,
		};
	};
	const fetchQuestionCounts = async (interviewId) => {
		const questions = await getQuestions(interviewId);
		return questions.length;
	};
	const fetchCountsFromId = async (ivdata) => {
		const applicantCountPromises = ivdata.map((interview) =>
			fetchApplicantCounts(interview.id)
		);
		const questionCountPromises = ivdata.map((interview) =>
			fetchQuestionCounts(interview.id)
		);
		const [applicantCountsArray, questionCountsArray] = await Promise.all([
			Promise.all(applicantCountPromises),
			Promise.all(questionCountPromises),
		]);
		const applicantCountsObject = {};
		const questionCountsObject = {};

		ivdata.forEach((interview, index) => {
			applicantCountsObject[interview.id] = applicantCountsArray[index];
			questionCountsObject[interview.id] = questionCountsArray[index];
		});
		return { applicantCountsObject, questionCountsObject };
	};
	useEffect(() => {
		fetchInterviews();
	}, []);
	const fetchInterviews = async () => {
		try {
			...
			const { applicantCountsObject, questionCountsObject } =
				await fetchCountsFromId(ivdata);
			setApplicantCounts(applicantCountsObject);
			setQuestionCounts(questionCountsObject);
		} catch (error) {
						...
						{interviews.map((interview) => (
							...
							>
								...
								<div className="p-3 sm:p-6">
									<div className="grid grid-cols-1 lg:grid-cols-[30%_30%_10%_15%_15%] gap-3 items-center">
										{/* Column 1 - Interview Info (30%) */}
										<div>
											...
											<div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
												<div className="flex items-center gap-2 text-gray-500">
													...
													<span className="text-sm sm:text-base">
														{questionCounts[interview.id] || 0} Questions
													</span>
												</div>
												<div className="flex items-center gap-2 text-gray-500">
													<IoPeopleOutline className="w-6 h-6 text-black" />
													<span className="text-sm sm:text-base">
														{applicantCounts[interview.id]?.total || 0}{" "}
														Applicants
													</span>
												</div>
											</div>
										...
										{/* Column 2 - Progress Boxes (30%) */}
										<div className="flex gap-3 sm:gap-4">
											<div className="p-3 sm:p-4 text-center bg-[var(--primary-green-light)] rounded-xl">
												<div className="font-semibold text-sm sm:text-base">
													Completed
												</div>
												<div className="text-lg sm:text-xl font-bold my-1">
													{applicantCounts[interview.id]?.completed || 0}
												</div>
											</div>
											<div className="p-3 sm:p-4 text-center bg-[var(--primary-purple-light)] rounded-xl">
												<div className="font-semibold text-sm sm:text-base">
													In Progress
												</div>
												<div className="text-lg sm:text-xl font-bold my-1">
											...
											<div className="p-3 sm:p-4 text-center bg-gray-200 rounded-xl">
												<div className="font-semibold text-sm sm:text-base">
													Not Started
												</div>
												<div className="text-lg sm:text-xl font-bold my-1">
													{applicantCounts[interview.id]?.notStarted || 0}
												</div>
											</div>
										</div>
										...
										<div className="flex justify-center">
											<span
												className={`px-3 py-1 text-sm sm:text-base font-semibold rounded-lg ${getStatusColor(
													interview.status
												)}`}
											>
												{interview.status}
										...
										{/* Column 4 - Applicant/Question Buttons (10%) */}
										...
										{/* Column 5 - Edit/Delete Buttons (20%) */}
										<div className="flex gap-5 justify-start mx-auto">
											<button
